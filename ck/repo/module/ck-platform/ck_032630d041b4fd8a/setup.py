...
def setup(i):
    ...
    if i.get('server_url') not in [None, '']: server_url=i['server_url']

    if server_url is None or server_url == '': server_url=config.CR_DEFAULT_SERVER_URL
    ...
    if i.get('server_user') not in [None, '']: server_user=i['server_user']

    if server_user not in [None, '']: cfg['server_user']=server_user

    # Server Pass ###########################################################        
    server_pass=cfg.get('server_pass','')

    if i.get('server_pass') not in [None, '']: server_pass=i['server_pass']

    if server_pass not in [None, '']: cfg['server_pass']=server_pass

    # Server Skip Certificate Validation ###########################################################        
    server_skip_validation=cfg.get('server_skip_validation','')

    if i.get('server_skip_validation') not in [None, '']: server_skip_validation=i['server_skip_validation']
